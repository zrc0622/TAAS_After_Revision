...
class HarfangEnv():
    ...
    def get_loc_diff(self, state):
        loc_diff = ((((state[0]-state[12]) * 10000) ** 2) + (((state[1]-state[13]) * 10000) ** 2) + (((state[2]-state[14]) * 10000) ** 2)) ** (1 / 2)
        return loc_diff

    def get_reward(self, state, action):
        reward = 0
        step_success = 0
        loc_diff = self.get_loc_diff(state)  # get location difference information for reward
...
class HarfangEnvBeta2New(HarfangEnvBeta1New):
    def __init__(self):
